...
5. **Маршрут** ✅: зажать точку на карте → пин Б → линия маршрута + панель (км/мин/Начать поездку).
6. **Полировка**: иконки, анимации, follow-режим камеры.

### Сделано в этапе 5 (маршрут)
- Long-press (0.6с) по карте → точка Б (`MapReader` + `proxy.convert`).
- `RouteService` (MKDirections, авто) → линия `MapPolyline`, расстояние, время в пути.
- `RoutePanel` (Liquid Glass) снизу: время, км, «Отмена» / «Начать поездку».
- «Начать поездку» прячет панель и показывает спидометр + «Завершить».
- ⏳ Настоящий turn-by-turn с голосом и поворотами — отдельный большой этап (отложено).
